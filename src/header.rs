...
impl Header {
    ...
    /// Returns a mutable reference to the underlying `tar_core::Header`.
    ///
    /// This is safe because `tar_core::Header` is `#[repr(transparent)]`
    /// over `[u8; 512]`, which is the same layout as our `bytes` field.
    #[allow(dead_code)]
    fn as_core_mut(&mut self) -> &mut tar_core::Header {
        // SAFETY: tar_core::Header is #[repr(transparent)] over [u8; 512],
        // which has identical layout and alignment to self.bytes.
        unsafe { &mut *(self.bytes.as_mut_ptr() as *mut tar_core::Header) }
    }

    /// Verify the header checksum using `tar_core`.
    ///
    /// This delegates to `tar_core::Header::verify_checksum()` and maps the
    /// error to `io::Error`.
    #[allow(dead_code)]
    fn verify_checksum_core(&self) -> io::Result<()> {
        self.as_core()
            .verify_checksum()
