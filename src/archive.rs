...
impl Archive<dyn Read + '_> {
    fn _entries<'a>(
        ...
        // Use permissive limits to match tar-rs's existing lenient behavior.
        // tar-rs historically accepted any archive without size/path limits.
...
impl<'a> EntriesFields<'a> {
    ...
    fn next_entry_raw(&mut self) -> io::Result<Option<Entry<'a, io::Empty>>> {
        ...
        loop {
            ...
            // Read the 512-byte header; EOF means end of archive.
            let mut read = 0;
            while read < header.as_bytes().len() {
                match (&self.archive.inner).read(&mut header.as_mut_bytes()[read..])? {
        ...
        loop {
            ...
            match event {
                ParseEvent::NeedData { min_bytes } => {
                    ...
                    debug_assert!(
                        min_bytes > cur_len,
                        "parser returned NeedData {{ min_bytes: {min_bytes} }} \
                         but buffer already has {cur_len} bytes"
                    );
                    self.buf.resize(min_bytes, 0);
                    match try_read_all(&mut &self.archive.inner, &mut self.buf[cur_len..]) {
                        Ok(true) => {
                            self.next += (min_bytes - cur_len) as u64;
                        }
                        Ok(false) => {
                ...
                ParseEvent::End { consumed } => {
                    if self.archive.inner.ignore_zeros {
                        ...
                        self.parser = Parser::new(Limits::permissive());
                        continue;
                    }
                    return Ok(None);
                }
    ...
    fn finish_entry(&mut self, meta: EntryMeta) -> io::Result<Option<Entry<'a, io::Empty>>> {
        let header_pos = self.next - meta.consumed as u64;
        let file_pos = self.next;
