...
const REVDEPS: &[RevDep] = &[
    RevDep {
        ...
        commit: "c63475b01db997f2085a2a95c7c756a935f595e1",
        toolchain: None,
    },
    RevDep {
        name: "cargo-vendor-filterer",
        repo: "https://github.com/coreos/cargo-vendor-filterer.git",
        commit: "32804ddef5be3977b86ca16da51504c17ff7cdfc",
        toolchain: None,
    },
    RevDep {
        name: "crates-io",
        repo: "https://github.com/rust-lang/crates.io.git",
        commit: "e482ed3da791735f37489cb9e6410a3b768d51f1",
...
enum Cli {
    ...
    ///
    /// Clones known reverse dependencies at pinned commits, patches them
...
fn main() -> Result<()> {
    let cli = Cli::parse();
    match cli {
        Cli::RevdepTest { targets, self_test } => {
            let tar_rs_root = project_root()?;
            if self_test {
                run_self_test(&tar_rs_root)
            } else {
                let revdeps = resolve_targets(&targets)?;
...
fn clone_at_commit(sh: &Shell, repo: &str, commit: &str, dest: &Path) -> Result<()> {
    if dest.join(".git").is_dir() {
        let current = cmd!(sh, "git -C {dest} rev-parse HEAD").read()?;
        if current.trim() == commit {
            println!(":: {}: already at {commit}", dest.display());
            return Ok(());
        }
        println!(
            ":: {}: wrong commit ({}), re-fetching...",
            dest.display(),
            current.trim()
        );
        cmd!(sh, "git -C {dest} fetch origin {commit}").run()?;
        cmd!(sh, "git -C {dest} checkout {commit}").run()?;
    } else {
        println!(":: Cloning {repo} at {commit}...");
...
fn test_cargo(sh: &Shell, tar_rs_root: &Path, revdep: &RevDep) -> Result<()> {
    ...
    clone_at_commit(sh, revdep.repo, revdep.commit, &dest)?;
